<script lang="ts">
	...
	import { getFavorites, clearFavorites, removeFavorite } from '$lib/services/favorites';
	import { goto } from '$app/navigation';
	import { browser } from '$app/environment';

	let records = $state<{ problem: AnyProblem; timestamp: number }[]>([]);

	$effect(() => {
		if (browser) {
			records = getFavorites();
		}
	});
	...
	function formatProblem(problem: AnyProblem): string {
		if ('type' in problem) {
			...
			if (problem.type === 'remainder') {
				...
				return `${p.dividend} ÷ ${p.divisor} = ___ ... ___`;
			}
		}
	...
	function handleReview() {
		goto('/practice?mode=favorite-review');
	}
</script>

<svelte:head>
	<title>收藏题库 - 小小数学家</title>
</svelte:head>
...
<style>
	.favorites-page {
		min-height: 100vh;
		background: linear-gradient(180deg, #fff8e1, #e3f2fd);
